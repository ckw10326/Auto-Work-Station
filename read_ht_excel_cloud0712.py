...
#輸入value，輸出key
def get_mapped_field(input_field):
...
def main():
    ...
    read_ht_make_list2(convert_xls)
    return None
...
def to_df_analyze(excel_path, combined_csv_path = "/workspaces/Auto-Work-Station/01Class/data.csv"):
    ...
    # 指定要移動到第一列和第二列的表頭
    code_headers = ['No', 'drawing_no_value', 'drawing_title_value', 'drawing_vision_value', 'letter_num_value', 'letter_date_value', 'letter_titl_value', 'file_path']
    zh_headers = ['批次序號', '圖號', '圖名', '版次', '來文號碼', '來文日期', '來文名稱', '路徑']
    vender1_zh_headers = ['', 'CLIENTDOCNO', 'DESCRIPTION', 'DOCVERSIONDESC', 'TRANSMITTALNO', 'REVDATE', 'DESCRIPTION', '']
    vender2_zh_headers = ['', 'DOCVERSIONDESC', 'DOCCLASS', '', '', 'PLANNEDCLIENTRETURNDATE', '', '']
